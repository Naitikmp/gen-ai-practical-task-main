...
async def word_game_node(state: WorkflowState):
    ...
    try:
        ...
        if not current_game_type:
            ...
            return None
        if current_game_type != "word_game":
            logger.error(f"Game type '{current_game_type}' is not 'word_game'.")
            return None
        llm = ChatOpenAI(
            model="gpt-4o",
            temperature=0.0,
        ...
        prompt_template = word_game_prompt.format_messages(
            word_game_list=word_game_list,
